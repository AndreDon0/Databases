...
Основное меню (выберите цифру):
    ...
    2 — применить миграции Alembic, очистить таблицы помещений/стеллажей и заполнить примерами;
    9 — выход."""
        print(menu)
    ...
    def after_main_menu(self, next_step: str) -> str:
        if next_step == "2":
            print(
                "Внимание: будут удалены все строки из public.rack и public.room. "
                "Если к стеллажам есть ссылки из других таблиц (не из этого варианта), операция не выполнится."
            )
            db_reset_and_seed()
            print("Миграции применены, таблицы очищены и заполнены заново.")
            return "0"
